...
clear
data_dir = '/Volumes/MelSSD/runs/base_nexp/eval-vn-120000';
save_dir = fullfile(data_dir, 'opt-eval');
% m_train = fullfile(data_dir, '../mat/dm-train-120000.mat');
m_val = load(fullfile(data_dir, 'test-syn-0.0-0.7-patchy-val_MS_6comb.mat'))
% m_val = load(fullfile(data_dir, 'test-syn-0.0-0.0-test-fullpipeline_val_64_6comb_64_imgs.mat'));
% m_val = fullfile(data_dir, 'test-syn-0.0-0.7-patchy-testset_ideal_MS_32_imgs.mat');
data = load('/Users/melaniebernhardt/Desktop/MScThesis/USImageReconstruction/data/val_MS_6comb.mat');
addpath(genpath('/Users/melaniebernhardt/Desktop/MScThesis/Generic_Data_Processing_Structure'));
mkdir(save_dir);
...
D = full(D);
%%
[data_res, img_res, d_grad, r_grad] = compute_all_residuals(L, D, m_val, permute(m_val.gt_slowness, [1,3,2]));
...
%%
function [data_res, img_res, d_grad, r_grad] = compute_all_residuals(L, D, m_val_ray, xstar)
