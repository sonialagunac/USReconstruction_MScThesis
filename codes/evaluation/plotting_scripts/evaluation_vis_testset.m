...
clear
%data_dir = '/Volumes/MelSSD/runs/7_10_filters/eval-vn-120000/';
%data_dir = '/scratch_net/biwidl307/sonia/USImageReconstruction-Melanie/runs/32_mix_ideal_20l_reg/eval-vn-120000/';
data_dir = 'C:\Users\sonil\PycharmProjects\MScThesis_CAIM\USImageReconstruction\runs\32_mix_ideal_20l_reg_mix_L2\eval-vn-120000\';
...
addpath(genpath('/scratch_net/biwidl307/sonia/USImageReconstruction-Melanie/code/'))
%%
save_dir = 'plots_testset';
%data_dir='C:\Users\sonil\PycharmProjects\MScThesis_CAIM\USImageReconstruction\codes\evaluation\plotting_scripts_old\'
...
%filename = 'clean_testset_ideal_4angles_84_32_imgs.mat';
...
% masks = ["plain", "patchy"];
%data_dir='C:\Users\sonil\PycharmProjects\MScThesis_CAIM\USImageReconstruction\codes\evaluation\plotting_scripts_old\'
data_dir = 'C:\Users\sonil\PycharmProjects\MScThesis_CAIM\USImageReconstruction\runs\32_mix_ideal_20l_reg_mix_L2_tau2.5\eval-vn-120000\';
fileID = 'kk'
noises = ["0.0"];
masks = ["test"];
fprintf(fileID, sprintf('\n %s 005\n', data_name));
...
% %% Detail plots with measurements
% clf;
% mkdir(fullfile(data_dir, save_dir, 'detail_plots_005_patchy'));
% mkdir(fullfile(data_dir, save_dir, 'detail_plots_005_incoherent'));
% mkdir(fullfile(data_dir, save_dir, 'detail_plots_00_patchy'));
% mkdir(fullfile(data_dir, save_dir, 'detail_plots_00_incoherent'));
%  for p=1:Nimgs
%     %figure();
%     set(gcf, 'Position',  [1, 1, 600+300*NA, 600]);
%     plot_all_both(p, val_images_syn_005_patchy, val_images_ideal_005_patchy, NA);
%     suptitle({sprintf('Validation image %d', p), ' '});
%     name = sprintf('detail_plots_005_patchy/both_val_im_%d.png', p);
%     saveas(gcf,fullfile(data_dir, save_dir, name));
%  end
%  
% clf;
%  for p=1:Nimgs
%     set(gcf, 'Position',  [1, 1, 600+300*NA, 600]);
%     plot_all_both(p, val_images_syn_00_patchy, val_images_ideal_00_patchy, NA);
%     suptitle({sprintf('Validation image %d', p), ' '});
%     name = sprintf('detail_plots_00_patchy/both_val_im_%d.png', p);
%     saveas(gcf,fullfile(data_dir, save_dir, name));
%  end
%  
%  clf;
%  for p=1:Nimgs
%     set(gcf, 'Position',  [1, 1, 600+300*NA, 600]);
%     plot_all_both(p, val_images_syn_005_incoh, val_images_ideal_005_incoh, NA);
%     suptitle({sprintf('Validation image %d', p), ' '});
%     name = sprintf('detail_plots_005_incoherent/both_val_im_%d.png', p);
%     saveas(gcf,fullfile(data_dir, save_dir, name));
%  end
%  
%   clf;
%  for p=1:Nimgs
%     set(gcf, 'Position',  [1, 1, 600+300*NA, 600]);
%     plot_all_both(p, val_images_syn_00_incoh, val_images_ideal_00_incoh, NA);
%     suptitle({sprintf('Validation image %d', p), ' '});
%     name = sprintf('detail_plots_00_incoherent/both_val_im_%d.png', p);
%     saveas(gcf,fullfile(data_dir, save_dir, name));
%  end
%  
